...
class Media{
    ...
    public function showMedia($item){
        $codes=array('/\$id/','/\$title/','/\$fileurl/','/\$tag/', '/\$showorder/', '/\$description/', '/\$caption/', '/\$alt/');
        
        foreach ($this->mediaGallery as $slide){
            $replace_code=array($slide['id'],$slide['title'], $slide['fileurl'],$slide['tag'], $slide['showorder'], $slide['description'], $slide['caption'], $slide['alt']);
