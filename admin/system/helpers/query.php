...
class Query {
    ...
    public static function createSchema($table, $fields = array(), $options = array()) {
        ...
        foreach ($options as $option => $value)
            $_options .= "<$option><![CDATA[$value]]></$option>";
        
        
        // Create table fields 
        $_fields = '<fields>';
        foreach ($fields as $field => $type)
            $_fields .= "<$field><![CDATA[$type]]></$field>";
        $_fields .= '</fields>';
    ...
    public function htmlTable($fields, $options, $showoptions = true) {
        ...
        foreach ($fields as $item => $value) {
            ...
            if (count($widths) > 0)
                $this->tableOutput .= " style='width:" . $widths[$count] . "%' ";
    ...
    public function htmlTableRow($row, $options, $showoptions = true, $menu = array()) {
        ...
        foreach ($fields as $item) {
            $this->tableOutput .= '<td>';
            if (isset($options['indent']) && $options['indent'] == $item)
                $this->tableOutput .= "&nbsp;&nbsp;-&nbsp;&nbsp;";
            $options['id'] = $row['id'];
            $this->tableOutput .= $this->getField($item . '-' . $row['id'], '', $this->tableFields[$item], $options, $row[$item]);
            $this->tableOutput .= '</td>';
    ...
    public function addRecordForm() {
        ...
        foreach ($fieldTypes as $name => $val) {
            if (!isset($_POST['post-' . $name]) && $val == "checkbox")
                $_POST['post-' . $name] = 'false';
        ...
        if ($ret) {
            ...
            $this->generateCacheFile();
            $this->getCache();
            Core::addAlert(Form::showAlert('success', __("CREATED", array(
                ":record" => $data['id'],
                ":type" => "Record"
            ))));
            return true;
        } else {
            Core::addAlert(Form::showAlert('alert', __("CREATEDFAIL", array(
                ":record" => $data['id'],
                ":type" => "Record"
            ))));
            return false;
